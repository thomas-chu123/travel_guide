...
export function App() {
  ...
  const cardTitle = (item: LocationFeature) => item.properties.category === "exhibition" ? localizedLocation(item, locale) || t.venue : localizedName(item, locale);
  const cardSubtitle = (item: LocationFeature) => item.properties.category === "exhibition" ? localizedName(item, locale) : localizedLocation(item, locale) || categoryLabels[category];
